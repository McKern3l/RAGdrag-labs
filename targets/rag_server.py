"""Bundled RAG test target for ragdrag security assessment.

This is an intentionally vulnerable RAG server for testing ragdrag against
a realistic target. It runs a FastAPI app backed by ChromaDB for vector
storage and Ollama for LLM generation. The knowledge base contains
deliberately planted sensitive data (fake credentials, infrastructure
details, PII) to validate exfiltration and fingerprinting techniques.

DO NOT deploy this server in any environment other than local testing.
The knowledge base contains intentionally insecure test data.
...
CHROMA_DIR = os.environ.get("CHROMA_DIR", "/tmp/ragdrag-dogfood-chroma")
